...
{
#ifdef QT_GUI
    // ensure we leave the Qt main loop for a clean GUI exit (Shutdown() is called in paycoin.cpp afterwards)
...
void Shutdown(void* parg)
{
    ...
    if (fFirstThread)
    {
        ...
        fExit = true;
#ifndef QT_GUI
        // ensure non-UI client gets exited here, but let Paycoin-Qt reach 'return 0;' in paycoin.cpp
        exit(0);
#endif
    }
    else
...
int main(int argc, char* argv[])
{
    ...
    // Connect paycoind signal handlers
    noui_connect();
...
{
    uiInterface.ThreadSafeMessageBox(str, _("PayCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("PayCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9288  or testnet: 19288)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9289 or testnet: 19289)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Paycoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  PayCoin is probably already running."), strDataDir.c_str()));
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("PayCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    // requested to kill paycoin-qt during the last operation. If so, exit.
