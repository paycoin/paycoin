...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 5 );//start diff is 1 / 2^12, if modify 20, start diff is 0.25
static CBigNum bnProofOfStakeLimit(~uint256(0) >> 24);
static CBigNum bnProofOfStakeHardLimit(~uint256(0) >> 25); // disabled temporarily, will be used in the future to fix minimum PoS difficulty at 0.25
...
int64 nChainStartTime = 1375785915;//2013/08/06
int nCoinbaseMaturity = 40; //mining need 60 confirm
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // PayCoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal paycoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PayCoin
    bool fStrictPayToScriptHash = true; // Always active in PayCoin
    ...
    // ppcoin: fees are not collected by miners as in paycoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PayCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "PayCoin - The future of online payment! Release August 6th";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:Paycoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x0daac50ec1531b4ff065395fe2ec288b4420ab103acdf40199348fb7d1b9a270"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("PaycoinMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PaycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("PaycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadPaycoinMiner(void* parg);

static bool fGeneratePaycoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void PaycoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("paycoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratePaycoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratePaycoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running PaycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadPaycoinMiner(void* parg)
{
    ...
    try
    {
        ...
        PaycoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadPaycoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadPaycoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadPaycoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GeneratePaycoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratePaycoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratePaycoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d PaycoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadPaycoinMiner, pwallet))
                printf("Error: NewThread(ThreadPaycoinMiner) failed\n");
            Sleep(10);
        }
    }
}
